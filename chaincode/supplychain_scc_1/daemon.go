...
func handOffItem(
	...
	cmd := exec.Command("python3.7", "-u", "apps/fabric/src/supplychain/v1/hand_off_item.py", idxInputProvider, maskedInputProvider, idxOutputProvider, maskedOutputProvider, itemID, prevSeq, seq, sharePrevOutputProvider)
	cmd.Dir = "/usr/src/HoneyBadgerMPC"
	file, err := os.Create("/usr/src/HoneyBadgerMPC/log.txt")
	if err != nil {
		log.Fatal(err)
