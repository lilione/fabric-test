...
func (t *eq) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	if fn == "createInstance" && len(args) >= 3 {
		...
		memcellX := "auction" + args[0] + strconv.Itoa(len(LinRegInstance.X)) + "X_cell"
		...
		return shim.Success([]byte(memcellX))
		...
		if len(LinRegInstance.X) < LinRegInstance.EndPoints {
		...
		cells := (LinRegInstance.X)

		mpcOp(stub, "auction", LinRegInstance.Name, cells...)
